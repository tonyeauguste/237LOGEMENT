...
export default function FaqPage() {
  ...
  const tCat = useTranslations("FaqCategories");
  ...
  return (
    <div className="max-w-[760px] mx-auto px-[5%] pt-[90px] pb-20">
      <div className="text-center mb-[52px]">
        ...
        <Reveal delay={0.14}>
          <p className="text-muted text-[15px] mt-3.5 max-w-[480px] mx-auto">{t("subtitle")}</p>
        </Reveal>
      </div>
      ...
      <div className="bg-card2 border border-border rounded-2xl p-8 text-center mt-12">
        <h3 className="font-display text-[22px] font-bold text-text mb-2.5">{t("notFoundTitle")}</h3>
        <p className="text-muted text-sm mb-[22px]">{t("notFoundText")}</p>
        <div className="flex gap-3 justify-center flex-wrap">
          <Link href="/contact">
            <Button variant="gold">{t("writeButton")}</Button>
          </Link>
